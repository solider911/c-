...
{  
	...
	public:  
		...
		void on_message(websocketpp::connection_hdl, ws_client::message_ptr msg) {  
			if (msg->get_opcode() == websocketpp::frame::opcode::text) {  
				m_msg_recv = msg->get_payload();
			}  
			else {  
				m_msg_recv = websocketpp::utility::to_hex(msg->get_payload());
			}  

			m_messages.push_back("<< " + m_msg_recv); 

			// 此处可自行处理

			std::cout << m_msg_recv.c_str() << std::endl;
		}  
	...
	websocket_endpoint::websocket_endpoint(){  
		...
		g_wsEndPoint.clear_access_channels(websocketpp::log::alevel::frame_payload);
	...
	void websocket_endpoint::show_recv()
	{
		std::cout << g_wsClientConnection->get_message().c_str() << std::endl;
	}
